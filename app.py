...
def get_stock_price_tradier(symbol):
    ...
    try:
        ...
        if 'quotes' in data and 'quote' in data['quotes']:
            ...
            description = quote.get('description', symbol)  # Fall back to symbol if no description
        ...
        if symbol:
            try:
                ...
                if not current_price:
                    ...
                    flash(f'Error: Could not get current price for {symbol}. Please check your Tradier API token.', 'danger')
                ...
                if calls is not None and puts is not None and not calls.empty and not puts.empty:
                    ...
                    if selected_date and selected_date in expirations:
                        ...
                        context['selected_date'] = expirations[0]
                else:
                    flash('Error: Could not get options data from Tradier. Please check your API token.', 'danger')
...
def search_stocks():
    ...
    most_common = [
        'AAPL', 'MSFT', 'SPY', 'TSLA', 'NVDA', 'AMZN', 'GOOGL', 'META', 'QQQ', 'JPM', 'V', 'MA', 'BAC', 'NFLX', 'AMD', 'DIS', 'WMT', 'BRK.B', 'XOM', 'UNH', 'VTI', 'VOO', 'IWM', 'DIA', 'GS', 'PYPL', 'COST', 'HD', 'T', 'PFE', 'MRK', 'KO', 'PEP', 'MCD', 'NKE', 'SBUX', 'INTC', 'CSCO', 'ORCL', 'CRM', 'ADBE', 'IBM', 'TXN', 'QCOM', 'MU', 'WFC', 'C', 'AXP', 'MS', 'GME', 'AMC', 'BB', 'NOK', 'F', 'GE', 'PLTR', 'NIO', 'RIOT', 'MARA'
    ]
...
def test_options(symbol):
    ...
    result = {
        'symbol': symbol,
        'tradier_configured': TRADIER_API_KEY != 'your_tradier_token_here' and TRADIER_API_KEY,
        'tradier_result': None,
        'errors': []
    }
