...
# Application logging to file for debugging
try:
    import logging
    from logging.handlers import RotatingFileHandler
    debug_handler = RotatingFileHandler('/var/log/trading-analysis/app_debug.log', maxBytes=2_000_000, backupCount=3)
...
# Global error handler to capture and log any unhandled exceptions
@app.errorhandler(Exception)
def handle_any_exception(e):
    try:
        with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
            _f.write(f"unhandled exception: {e}\n")
    except Exception:
        pass
    try:
        import traceback
...
def login():
    try:
        try:
            with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                _f.write(f"login: start method={request.method}\n")
        except Exception:
            pass
        if current_user.is_authenticated:
            try:
                with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                    _f.write("login: already authenticated -> redirect index\n")
            except Exception:
                pass
        ...
        try:
            with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                _f.write(f"login: validate_on_submit={valid} errors={form.errors}\n")
        except Exception:
            pass
        if valid:
            try:
                with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                    _f.write(f"login: querying user {form.username.data}\n")
            except Exception:
                pass
            user = User.query.filter_by(username=form.username.data).first()
            try:
                with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                    _f.write(f"login: user_found={bool(user)}\n")
            except Exception:
                pass
            if user and user.check_password(form.password.data):
                try:
                    with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                        _f.write("login: password ok; calling login_user\n")
                except Exception:
                    pass
                login_user(user)
                ...
                if 'pending_trade' in session:
                    try:
                        with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                            _f.write("login: pending_trade present -> redirect add_trade\n")
                    except Exception:
                        pass
                ...
                try:
                    with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                        _f.write(f"login: redirecting to next={next_page}\n")
                except Exception:
                    pass
            ...
            try:
                with open('/var/log/trading-analysis/app_debug.log', 'a') as _f:
                    _f.write("login: invalid credentials\n")
            except Exception:
                pass
