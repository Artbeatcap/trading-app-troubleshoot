...
from emails import send_confirmation_email, send_daily_brief_to_subscribers, send_welcome_email
from market_brief_generator import send_market_brief_to_subscribers

def test_subscription_flow():
    """Test the complete subscription flow"""
    print("🧪 Testing Newsletter Subscription Flow")
    print("=" * 50)
    ...
    with app.app_context():
        ...
        # Test 5: Send daily brief
        print("\n5. Sending daily brief...")
        try:
            success_count = send_market_brief_to_subscribers()
            print(f"✅ Daily brief sent to {success_count} subscribers")
        except Exception as e:
            print(f"❌ Failed to send daily brief: {str(e)}")
    ...
    with app.app_context():
        ...
        if total > 0:
            ...
            for sub in recent:
                ...
                print(f"  {sub.email} - {sub.name} - {status}")
